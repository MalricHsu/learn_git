...
test("curriculum contains seven chapters and exactly 38 terminal challenges", () => {
  assert.equal(chapters.length, 7);
  assert.equal(challenges.length, 38);
  assert.equal(challenges.filter((challenge) => challenge.final).length, 7);
  assert.equal(challenges.filter((challenge) => !challenge.final).length, 31);
...
test("every challenge has complete game and teaching configuration", () => {
  assert.equal(new Set(challenges.map((challenge) => challenge.id)).size, 38);
  for (const challenge of challenges) {
    ...
    for (const beat of ["opening", "stakes", "payoff"]) {
      assert.ok(
        challenge.story[beat].length >= 35,
        `${challenge.id} ${beat} is substantial`,
      );
    }
    assert.ok(challenge.story.prompt.length >= 35, `${challenge.id} prompt`);
    assert.ok(challenge.story.pressure.length >= 25, `${challenge.id} pressure`);
...
test("terminal challenges follow a new-hire workplace story", () => {
  ...
  assert.ok(challenges.every((challenge) => !challenge.task.zh.includes("undefined")));
  assert.ok(challenges.some((challenge) => challenge.story.zh.includes("小安")));
});
