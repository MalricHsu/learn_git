...
<template>
  <section class="viz-section timeline">
    <header><span>Commit Timeline</span><b>提交時間軸</b></header>
...
<style scoped>
.viz-section{padding:22px;border:1px solid var(--border);border-radius:12px;background:var(--paper-2)}header{display:flex;justify-content:space-between;font-family:var(--mono);font-size:10px;letter-spacing:.13em;text-transform:uppercase}header b{font-family:var(--serif-tc);font-weight:400}.empty{font-family:var(--serif-tc);color:var(--ink-faint);font-size:11px;margin:18px 0 0}ol{list-style:none;margin:18px 0 0;padding:0}li{display:grid;grid-template-columns:42px 1fr;gap:12px;padding:12px 0;border-top:1px solid var(--border)}time,code,small{font-family:var(--mono);font-size:9px;color:var(--ink-faint)}li b{display:block;font-family:var(--display);font-size:13px;margin-bottom:3px}code{margin-right:8px;color:var(--warning)}em{font-style:normal;color:var(--primary)}.commit-enter-active{transition:all 260ms ease}.commit-enter-from{opacity:0;transform:translateY(-8px)}@media(prefers-reduced-motion:reduce){.commit-enter-active{transition:none}}
</style>
